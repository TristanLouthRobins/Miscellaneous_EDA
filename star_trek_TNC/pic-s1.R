...
# Picard font-face is likely Din Condensed Bold
a

# create a vector of image 'headshots' of DS9 characters -----------------------
images_tng <- c(
  "star_trek_TNC/images/headshots/pic/dahj.jpg",
  ...
  "star_trek_TNC/images/headshots/pic/rikertroi.jpg",
...
MVC_leaderboard <- 
  ...
  mutate(img = ifelse(Character == "DAHJ", images[1], 
                      ifelse(Character == "HUGH", images[2], 
                             ifelse(Character == "JURATI", images[3],
                                    ifelse(Character == "LARIS", images[4],
                                           ifelse(Character == "NECKLACE", images[5], 
                                                  ifelse(Character == "ORCHIDS", images[6], 
                                                         ifelse(Character == "PICARD", images[7], 
                                                                ifelse(Character == "RAFFI", images[8], 
                                                                       ifelse(Character == "TROI-RIKERS", images[9], 
                                                                              ifelse(Character == "SEVEN", images[10],
                                                                                     images[10])))))))))))
...
ggsave("star_trek_TNC/exports-pic-s1/pic-final_s1_scores.png",width = 48, height = 24, units = "cm", dpi = 100) 
...
MVC_leaderboard <- MVC_leaderboard %>% 
  mutate(div_col = ifelse(Character %in% c(""), "#B5A424", # Command
                          ifelse(Character %in% c(""), "#CD6363", # Ops / Security
                                 ifelse(Character %in% c("", ""), '#3399FF', '#4C4D47')))) # Sciences, if not: Civilian/Other
...
MVC.tally <- 
  ...
  pivot_longer(cols = 3:12, names_to = "Character", values_to = "count") %>% 
  group_by(Character) %>% 
...
sel_chr <- c("SEVEN", "RAFFI", "PICARD", "TROI-RIKERS", "ORCHIDS", "LARIS", "HUGH", "NECKLACE", "JURATI", "DAHJ")
...
# MVC_plt - 
MVC_ranks_plt <- hlt_rank_plt(c("SEVEN"))
...
ggsave("star_trek_TNC/exports-pic-s1/tng_s1_MVC-wotitle.png",width = 48, height = 24, units = "cm",  dpi = 100) 
