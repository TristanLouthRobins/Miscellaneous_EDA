...
var.cmd <- c("Picard", "Riker", "Geordi")
var.ops <- c("O'Brien", "Data", "Yar", "Worf")
var.sci <- c("B.Crusher", "Troi")
var.civ <- c("W.Crusher")
var.oth <- c("Other")
...
images_tng <- c(
  "star_trek_TNC/images/headshots/tng-s1/picard.jpg",
  "star_trek_TNC/images/headshots/tng-s1/riker.jpg",
  "star_trek_TNC/images/headshots/tng-s1/data.jpg",
  "star_trek_TNC/images/headshots/tng-s1/worf.jpg",
  "star_trek_TNC/images/headshots/tng-s1/yar.jpg",
  "star_trek_TNC/images/headshots/tng-s1/bcrusher.jpg",
  "star_trek_TNC/images/headshots/tng-s1/wcrusher.jpg",
  "star_trek_TNC/images/headshots/tng-s1/troi.jpg",
  "star_trek_TNC/images/headshots/tng-s1/laforge.jpg",
  "star_trek_TNC/images/headshots/tng-s1/obrien.jpg",
  "star_trek_TNC/images/headshots/tng-s1/other.jpg"
)
...
MVC_leaderboard <- 
  ...
  mutate(img = ifelse(Character == "B.CRUSHER", images[6], 
                      ifelse(Character == "DATA", images[3], 
                             ifelse(Character == "LA FORGE", images[9],
                                    ifelse(Character == "O'BRIEN", images[10],
                                           ifelse(Character == "OTHER", images[11], 
                                                  ifelse(Character == "PICARD", images[1], 
                                                         ifelse(Character == "RIKER", images[2], 
                                                                ifelse(Character == "TROI", images[8], 
                                                                       ifelse(Character == "W.CRUSHER", images[7], 
                                                                              ifelse(Character == "WORF", images[4],
                                                                                     images[11])))))))))))
...
ggsave("star_trek_TNC/exports-tng-s1/tng-final_s1_scores.png",width = 48, height = 24, units = "cm", dpi=100) 
...
MVC_leaderboard <- MVC_leaderboard %>% 
  mutate(div_col = ifelse(Character %in% c("PICARD", "RIKER", "LA FORGE", "O'BRIEN"), '#CD6363', # Command
                          ifelse(Character %in% c("DATA", "YAR", "WORF"), '#B5A424', # Ops / Security
...
sel_chr <- c("PICARD", "W.CRUSHER", "TROI", "B.CRUSHER", "DATA", "JAKE", "WORF", "LA FORGE", "RIKER")
...
hlt_rank_plt <- function(selection){
  ...
  ggplot() +
    ...
    scale_x_continuous(limits = c(0, 26), breaks = c(1:25)) 
} 

# MVC_plt - highlight the top 3 MVCs for this season -----
MVC_ranks_plt <- hlt_rank_plt(c("PICARD", "W.CRUSHER", "TROI", "B.CRUSHER", "RIKER", "WORF", "LA FORGE", "DATA"))
...
ggsave("star_trek_TNC/exports-tng-s1/tng_s1_MVC-wotitle.png",width = 48, height = 24, units = "cm", dpi=100) 
...
ggsave("star_trek_TNC/exports-tng-s1/tng-final_s1_cluster.png", width = 36, height = 24, units = "cm", dpi=100) 
ggsave("star_trek_TNC/exports-tng-s1/tng-final_s1_cluster-square.png", width = 36, height = 36, units = "cm", dpi=100) 
...
ggsave("star_trek_TNC/exports-tng-s1/final_s1_all_scores.png",width = 24, height = 24, units = "cm", dpi=100) 
...
ggsave("star_trek_TNC/exports-tng-s1/final_s1_all_order_by_scores.png",width = 24, height = 24, units = "cm", dpi=100) 
...
ggsave("star_trek_TNC/exports-tng-s1/top_3_scores_facet.png",width = 24, height = 8, units = "cm", dpi=100) 
...
ggsave("star_trek_TNC/exports-tng-s1/top_episode.png",width = 12, height = 12, units = "cm", dpi=100) 

# patch EVERYTHING together ----------------------------------------------------
font_add("Star Trek TNG-Title", "/Users/tristanlouth-robins/Library/Fonts/Star Trek TNG-Title Regular.ttf")
showtext::showtext_auto()

tng_summary <- str_wrap(glue("The hosts Season {data$Season[1]} scores matched or were very close on {compare.AndyMatt$count[2]} occassions, {compare.AndyMatt$prop[2]}% of the time 
                                      and Andy consistently scored higher than Matt {scored.AndyMatt$count[1]} times ({scored.AndyMatt$prop[1]}%) compared to only {scored.AndyMatt$count[3]} 
                                      instances when Matt scored higher than Andy. Their average scores were close to each otherwith: {round(summary_stats$mean[2],1)} (Andy) 
                                      ...
                                      {compare.TNCIMDB$prop[2]}% of the time and the IMDb scores were significantly higher than the joint TNC score the majoprity of the time ({scored.TNCIMDB$count[1]}/25, 
                                      {scored.TNCIMDB$prop[1]}%) compared to only {scored.TNCIMDB$count[2]} instances when the joint TNC score was marginally higher than that of IMDb ('11001001' and 'Conspiracy').
                                      The average scores for this season were: {round(summary_stats$mean[3],1)} (TNC) and {round(summary_stats$mean[1],1)} (IMDb)"), 200)


# set up a base layer --
title <- "STAR TREK: THE NEXT CONVERSATION - THE NEXT GENERATION: SEASON 1"
caption <- "BROUGHT TO YOU BY TRISTAN LOUTH-ROBINS. GITHUB: https://github.com/TristanLouthRobins"
plot.bg <- "#000000"

base.tng <- ggplot() +
  labs(title = title,
       subtitle = tng_summary,
       caption = caption) +
  theme_trek() +
...
ggsave("star_trek_TNC/exports-tng-s1/tnc_tng_s1.png", plot = tnctngs1plt, width = 80, height = 60, units = "cm", dpi = 100) 
