...
# Latest update: v1.2.3 (15 Oct 2023) ------------------------------------------
library(tidyverse)
...
var.sci <- c("Dax", "Bashir", "Melora")
var.civ <- c("Jake", "Borom", "Nog", "Keiko", "Quark", "Garak", "Leeta", "Kassidy")
...
images <- c(
  "images/headshots/ds9/s2/bashir.jpg",
  "images/headshots/ds9/s2/dax.jpg",
  "images/headshots/ds9/s2/jake.jpg",
  "images/headshots/ds9/s2/kira.jpg",
  "images/headshots/ds9/s2/obrien.jpg",
  "images/headshots/ds9/s2/odo.jpg",
  "images/headshots/ds9/s2/quark.jpg",
  "images/headshots/ds9/s2/sisko.jpg",
  "images/headshots/ds9/s2/nog.jpg",
  "images/headshots/ds9/s2/garak.jpg",
  "images/headshots/ds9/s2/other.jpg",
  "images/headshots/ds9/s2/rom.jpg",
  "images/headshots/ds9/s2/melora.jpg",
  "images/headshots/ds9/s2/borum.jpg"
)

# apply cropcircles to create circular images representing MVCs ----------------
image_df <- tibble(y = 1:14, images = images) %>% 
                                                                ...
                                                                ifelse(Character == "SISKO", images[8], 
                                                                       ifelse(Character == "NOG", images[9], 
                                                                              ifelse(Character == "GARAK", images[10],
                                                                                     ifelse(Character == "OTHER", images[11],
                                                                                            ifelse(Character == "ROM", images[12],
                                                                                                  ifelse(Character == "MELORA", images[13],
                                                                                                         ifelse(Character == "BORUM", images[14],
                                                                                     images[14])))))))))))))
         ))
...
ggsave("exports-ds9-s2/ds9-final_s2_scores.png",width = 48, height = 24, units = "cm", dpi = 100) 
...
MVC_leaderboard <- MVC_leaderboard %>% 
  mutate(div_col = ifelse(Character %in% c("O'BRIEN", "ODO"), "#B5A424", # Command
                          ifelse(Character %in% c("SISKO", "KIRA"), "#CD6363", # Ops / Security
                                 ifelse(Character %in% c("", "DAX", "BASHIR", "MELORA"), '#3399FF', '#4C4D47')))) # Sciences, if not: Civilian/Other
...
(MVC_plt <- 
    ...
    scale_fill_identity() +
    geom_image(aes(x=count, y=fct_reorder(Character, count), image = img), size = 0.09) +
    geom_point(aes(x=count+1.5, y=fct_reorder(Character, count)), size = 10, colour ="#E7FFFF") +
    geom_text(aes(x=count+1.5, y=fct_reorder(Character, count), label=count, family = "Antonio"), size = 4, colour ="#000000") +
    # MVC 1
...
hlt_rank_plt <- function(selection){
  ...
  ggplot() +
    ...
    scale_x_continuous(limits = c(0, 14), breaks = c(1:10)) 
} 
...
ggsave("exports-ds9-s2/ds9_s2_MVC-wotitle.png",width = 48, height = 24, units = "cm",  dpi = 100) 
...
cluster_plot <- 
  ...
  annotate(geom = "text", x=-2.5, y=3.2 ,label=cluster_caption, size=5, colour="#E7FFFF", family = "Antonio", hjust=0, vjust=1, lineheight = 1.3) +
...
ggsave("star_trek_TNC/exports-ds9-s1/ds9-final_s1_cluster.png.png", width = 36, height = 24, units = "cm", dpi = 100) 
ggsave("star_trek_TNC/exports-ds9-s1/ds9-final_s1_cluster-square.png.png", width = 36, height = 36, units = "cm", dpi = 100) 
...
ggsave("star_trek_TNC/exports-ds9-s1/final_s1_all_scores.png",width = 24, height = 24, units = "cm", dpi=100) 
...
ggsave("star_trek_TNC/exports-ds9-s1/final_s1_all_order_by_scores.png",width = 24, height = 24, units = "cm", dpi=100) 
...
ggsave("star_trek_TNC/exports-ds9-s1/top_3_scores_facet.png",width = 24, height = 8, units = "cm", dpi = 100) 
...
ggsave("star_trek_TNC/exports-ds9-s1/top_episode.png",width = 12, height = 12, units = "cm", dpi=100) 

# patch EVERYTHING together ----------------------------------------------------
font_add("Star Trek TNG-Title", "/Users/tristanlouth-robins/Library/Fonts/Star Trek TNG-Title Regular.ttf")
showtext::showtext_auto()

summary_ds9.1 <- str_wrap(glue("The hosts Season {data$Season[1]} scores matched or were very close on {compare.AndyMatt$count[2]} occassions, {compare.AndyMatt$prop[2]}% 
                                      ...
                                      to only {scored.TNCIMDB$count[2]} instances when the joint TNC score was marginally higher than that of IMDb ('Emissary' and 'Battlelines').
                                      The average scores for this season were: {round(summary_stats$mean[3],1)} (TNC) and {round(summary_stats$mean[1],1)} (IMDb)"), 200)


# set up a base layer --
title <- "STAR TREK: THE NEXT CONVERSATION - DEEP SPACE NINE: SEASON 1"
caption <- "BROUGHT TO YOU BY TRISTAN LOUTH-ROBINS. GITHUB: https://github.com/TristanLouthRobins"
plot.bg <- "#000000"

base <- ggplot() +
  labs(title = title,
       subtitle = summary_ds9.1,
       caption = caption) +
  theme_trek() +
  theme(plot.title = element_text(family = "Federation", size = 48, colour = "#3399FF", margin=margin(0,0,10,0)),
...
ggsave("star_trek_TNC/exports-ds9-s1/tnc_ds9_s1.png", plot = tngds9s1plt, width = 80, height = 60, units = "cm", dpi = 100) 
